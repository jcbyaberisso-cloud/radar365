...
export async function getUpcomingMarketFeeds(sport:string,maxEvents=2,maxPrices=10,horizonHours=36){
 const events=await propLineRequest<Event[]>(`/sports/${encodeURIComponent(sport)}/events`);const now=Date.now(),end=now+horizonHours*3600000;
 const upcoming=events.filter(e=>{const t=eventTime(e);return t>=now-30*60000&&t<=end}).sort((a,b)=>eventTime(a)-eventTime(b)).slice(0,maxEvents);
 const feeds=[];
 for(const event of upcoming){
  ...
  const priority=['player_shots_on_target','player_shots','goalie_saves','anytime_goal_scorer','goal_or_assist','player_cards','team_corners','totals'];const selected:RadarPrice[]=[];const seen=new Set<string>();
  for(const market of priority){for(const p of all.filter(x=>x.market===market).sort((a,b)=>priceScore(b)-priceScore(a))){const k=`${p.market}|${p.player}|${p.outcome}`;if(seen.has(k))continue;seen.add(k);selected.push(p);if(selected.filter(x=>x.market===market).length>=2||selected.length>=maxPrices)break}if(selected.length>=maxPrices)break}
  feeds.push({event,bookmakers,bet365Available,prices:selected});
 }
 return feeds;
}
